...
const Active = () => {
  ...
  return (
    ...
    {DesignData.map((data, index) => {
     return (data.status === 'Active' && <div key={index} className='border-[1px] rounded-[15px] w-[100%]   flex flex-col justify-center space-y-3 p-[3%]'>
        
        <div className='flex justify-between'>
        <h1 className='font-medium text-[14px] text-[#1C1C1C]'>ID : {data.id}</h1>
        <h4 className='underline text-[#023020] text-[12px]'
        
        onClick={() =>handleViewDetails(data)}>View Details</h4>
        </div>
       ...
       <img className='w-[100%]' src={data.itemImg} alt="" />
        ...
        <div className='flex justify-center space-x-6'>
          <button  className='bg-[#023020] w-[100%] h-[40px] text-[white] font-poppins font-semibold text-[13px] sm:text-[10px] rounded-[10px]   '>Mark As Completed</button>
            </div>
