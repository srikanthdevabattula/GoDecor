...
const Accepted = () => {
  ...
  return (
    ...
    {ProductOrders.map((data, index) => {
     return (data.status === 'Accepted' && <div key={index} className='border-[1px] rounded-[15px] w-[100%]   flex flex-col justify-center space-y-3 md:space-y-2 p-[3%]'>
        <Card data={data}/>
        
        <div className='flex justify-center space-x-6'>
           <button  className='bg-[#023020] w-[100%] text-[white] font-poppins font-semibold text-[13px] sm:text-[10px] rounded-[10px] p-[10px_55px] sm:p-[3px_15px] '>Ready For Delivery</button>
            </div>
